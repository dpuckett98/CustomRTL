...
public class VerilogOrderedOperation implements GenerateVerilog {

	private String op;

	public VerilogOrderedOperation(String op) {
		this.op = op;
	}
	
	private Data getOut(Node n) {
		Util.ASSERT(n.getSinks().size() == 1, "ERROR: VerilogOrderedOperation should have one output: " + n.getFullName());
	...
	private Data getFirst(Node n) {
		...
		Util.ASSERT(hasSwitch, "ERROR: One of Ordered Operation's inputs must be tagged as first: " + n.getFullName());
		return swtch;
	}
	...
	private Data getSecond(Node n) {
		...
		Util.ASSERT(hasSwitch, "ERROR: One of Ordered Operation's inputs must be tagged as second: " + n.getFullName());
		return swtch;
	}
	...
	public String getAssignVals(Node n) {
		...
		return "assign " + out.getVerilog() + " = " + first.getVerilog() + " " + op + " " + second.getVerilog() + ";";
	}
